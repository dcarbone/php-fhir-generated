...
class ValueMinLengthRuleTest extends TestCase
{
    ...
    {
        $type = new MockStringPrimitiveType('string-primitive', 'on');
        $rule = new ValueMinLengthRule();
        $err = $rule->assert($type, 'value', 3, $type->getValue());
    ...
    {
        $type = new MockStringPrimitiveType('string-primitive', '');
        $rule = new ValueMinLengthRule();
        $err = $rule->assert($type, 'value', 3, $type->getValue());
        $this->assertNotNull($err);
    }
