...
trait MockTypeFieldsTrait
{
    protected array $_fields = [];

    public function __call(string $name, array $args): null|self|iterable|TypeInterface
    {
    ...
    public function __get(string $name): array|TypeInterface
    {
    ...
    {
        // compute this once.
        $mockElement = ($this instanceof ElementTypeInterface);
        ...
        foreach($fields as $field => $def) {
            ...
            if (!is_string($def['class']) || !is_a($def['class'], TypeInterface::class, true)) {
            ...
            $element = is_a($class, ElementTypeInterface::class, true);
            ...
            if ($primitive) {
                $this->_valueXMLLocations[$field] = ValueXMLLocationEnum::PARENT_ATTRIBUTE;
    ...
    protected function _xmlSerialize(XMLWriter $xw,
                                     SerializeConfig $config,
                                     null|ValueXMLLocationEnum $valueLocation = null): void
    {
        $mockPrimitiveContainer = ($this instanceof PrimitiveContainerTypeInterface);
        ...
        foreach($this->_fields as $field => $def) {
            ...
            if ($mockPrimitiveContainer) {
                if ('value' !== $field || ValueXMLLocationEnum::CONTAINER_ATTRIBUTE !== $valueLocation) {
                    continue;
                }
            } else {
                if ($primitiveContainer && ValueXMLLocationEnum::PARENT_ATTRIBUTE === $this->_valueXMLLocations[$field]) {
                    continue;
                }
            }
        ...
        foreach($this->_fields as $field => $def) {
            ...
            if ($collection) {
                ...
                if ($value->_nonValueFieldDefined() || ValueXMLLocationEnum::PARENT_ATTRIBUTE !== $this->_valueXMLLocations[$field]) {
                    $xw->startElement($field);
